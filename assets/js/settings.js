  ...
  root.replaceChildren(el('div', { class: 'setup' },
    el('div', { class: 'setup-col' },
      UI.section('Your company',
        doc.company.name?.trim() ? null : el('p', { class: 'warn' },
          'The quotation prints this as your letterhead. Fill it in before sending one out.'),
        UI.field('Company name', UI.textInput(doc.company.name, (v) => setCo({ name: v }),
          { placeholder: 'Your firm as it should appear on the quotation' })),
        UI.field('Address', UI.areaInput(doc.company.address, (v) => setCo({ address: v }),
          { rows: 3, placeholder: 'Street, town, district and PIN' })),
        UI.grid(2,
